...
vector<double> solver::startVelocitySun()
{
    vector<double> startVelocitySun(3);
    for (int i=0;i<total_planets;i++){
        for (int d=0;d<3;d++){
            startVelocitySun[d] -= all_planets[i].mass*all_planets[i].velocity[d];
        }
    }
    return startVelocitySun;
}

vector<double> solver::startPositionSun()
{
    vector<double> startPositionSun(3);
    for (int i=0;i<total_planets;i++){
        for (int d=0;d<3;d++){
            startPositionSun[d] -= all_planets[i].mass*all_planets[i].position[d];
        }
    }
    return startPositionSun;
}
